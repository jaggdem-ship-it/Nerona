/**
 * ============================================================================
 * WorldCollisionSystem.ts
 * ============================================================================
 * Core environmental physics mapping and collision resolution engine.
 *
 * Responsibilities:
 *   • Parse structural tile layout matrices (Layer 2 Walls/Pillars, Layer 3
 *     Exterior building frames) into a high-performance 2D spatial grid.
 *   • AABB vs Capsule and Circle vs Circle vector solvers with exact
 *     penetration-depth cancellation.
 *   • Structural slide-collision physics: velocity is projected onto the
 *     collision tangent plane, allowing smooth wall sliding.
 *
 * Design constraints:
 *   • Zero mesh references in the hot loop — pure scalar arithmetic only.
 *   • Zero object allocations inside per-frame collision resolution.
 *   • Spatial hash grid for O(1) broad-phase culling.
 *   • Iterative solver (3 passes) for robust corner / multi-contact handling.
 * ============================================================================
 */
...
export class WorldCollisionSystem {
  ...
  public buildFromTileMatrix(
    ...
    this.clear();
    // Allow overriding cell size per matrix (e.g. exterior frames use same grid)
    // We intentionally do NOT mutate this._cellSize so the system stays coherent
    // with the rest of the pipeline.  Callers should pass matching cellSize.
